<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\SoftDeletes;
use Illuminate\Support\Str;

class Artist extends Model
{
    use SoftDeletes;

    protected $fillable = ['image', 'name', 'description', 'link'];

    public static function boot()
    {
        parent::boot();

        static::saving(function ($model) {
            if ($model->isDirty('name') || !$model->exists) {
                $model->slug = Str::slug($model->name);

                // Check if the slug already exists and append a number if necessary
                $original_slug = $model->slug;
                $count = 2;
                while (static::whereSlug($model->slug)->where('id', '!=', $model->id)->exists()) {
                    $model->slug = $original_slug . '-' . $count++;
                }
            }
        });
    }

    public function agents()
    {
        return $this->hasMany(BookingAgent::class, 'artist_id', 'id');
    }

    public function downloads()
    {
        return $this->hasMany(ArtistDownload::class, 'artist_id', 'id');
    }

    public function releases()
    {
        return $this->hasMany(Release::class, 'artist_id', 'id');
    }
}
