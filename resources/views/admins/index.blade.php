            ...
            <div class="card-header">
                ...
                <div class="card-toolbar">
                    ...
                    <a href="{{ route('admin.admins.create') }}" class="btn btn-primary font-weight-bolder">
                        <i class="la la-plus"></i>New Admin</a>
                    <!--end::Button-->
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($admins as $key => $admin)
                            <tr>
                                ...
                                <td>{{ $admin->email }}</td>
                                <td class="d-flex">
                                    <a href="{{route('admin.admins.edit', $admin->id)}}" class="btn btn-icon btn-light-primary btn-sm mr-2" data-toggle="tooltip" title="Edit Admin!">
                                    ...
                                    <form id="delete-form-{{$admin->id}}" action="{{ route('admin.admins.destroy', $admin->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('{{$admin->id}}')" class="btn btn-icon btn-light-danger btn-sm mr-2" data-toggle="tooltip" title="Delete Admin!">
