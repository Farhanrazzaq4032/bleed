...
@section('section')
    <!--begin::Container-->
    <div class="container">
        <form action="{{ route('admin.artists.update', $artist->id) }}" method="POST" enctype="multipart/form-data" id="artist-form">
            @csrf
            @method('PUT')
            <!--begin::Card-->
            <div class="card card-custom mb-4">
                <div class="card-header">
                    <div class="card-title">
                        <h3 class="card-label">Update Artist</h3>
                    </div>
                ...
                <div class="card-body">
                    <div class="row form-group">
                                ...
                                <div class="col-lg-9 col-xl-6">
                                    <div class="image-input image-input-outline" id="kt_image_1">
                                        <div class="image-input-wrapper"
                                            style="background-image: url('{{ asset('uploads/artists/' . $artist->image) }}')">
                                        </div>
                                        <label
                        ...
                        <div class="col-6">
                            <div class="row">
                                <div class="col-12 my-2">
                                    <label>Name:</label>
                            <input type="text" name="artist_name" class="form-control" autocomplete="off"
                                placeholder="Enter Name" value="{{ old('artist_name') ?? $artist->name }}"  id="artist_name" required="">
                        ...
                        <input type="text" name="link" class="form-control" autocomplete="off"
                            placeholder="Enter Link" value="{{ old('link') ?? $artist->link }}"  id="link" required="">
                    ...
                    <div class="row form-group">
                        <div class="col-lg-12">
                            ...
                            <textarea class="form-control" id="artist_description" name="description" rows="6">{{ old('description') ?? $artist->description }}</textarea>
                ...
                <div class="card-body">
                    <div class="row">
                        <table class="table table-head-noborder table-responsive-sm purchase-table">
                            ...
                            <tbody id="tbl">
                                @forelse ($artist->agents as $item)
                                    <tr>
                                        <td><input type="text" name="name[]" class="form-control"
                                                value="{{ $item->name }}" autocomplete="off" placeholder="Enter Name">
                                        </td>
                                        <td><input type="text" name="email[]" class="form-control"
                                                value="{{ $item->email }}" autocomplete="off" placeholder="Enter Email">
                                        </td>
                                        <td><input type="text" name="phone[]" class="form-control"
                                                value="{{ $item->phone }}" autocomplete="off"placeholder="Enter Phone">
                                        </td>
            ...
            <div class="card card-custom">
                ...
                <div class="card-body">
                    ...
                    @if ($artist->downloads->count() != 0 && $artist->downloads != null && !empty($artist->downloads))
                        <div class="row form-group" id='prev_div'>
                            <div class="col-lg-6">
                                <div id="uploaded-files">
                                    ...
                                    <table class="table mt-2">
                                        ...
                                        <tbody>
                                            @foreach ($artist->downloads as $download)
                                                <tr id="down{{$download->id}}">
                                                    <th scope="row">{{ $download->file }}</th>
                                                    <td><button class="btn btn-icon btn-danger btn-sm btn-circle"
                                                            type="button"
                                                            onclick="deleteDownload('{{ $download->id }}')"><i
...
@push('scripts')
    ...
    <script>
        const deleteDownload = (id) => {
            let url = '{{ route('admin.artists.delete_download', ':id') }}';
     ...
     <script>
        $('#artist-form').submit(function(e) {
            ...
            if ($('#link').val() == '') {
                $('#link_error').text('Link is required');
                valid = false;
                if (!firstInvalidField) {
